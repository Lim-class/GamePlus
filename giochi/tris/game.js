...
function initGame() {
    // Se stavamo giocando online, interrompiamo l'ascolto in background
    if (gameMode !== 'online' && unsubscribeOnline) {
        unsubscribeOnline();
    ...
    if (gameMode === 'player-vs-ai') {
        statusDisplay.textContent = `È il turno di ${currentPlayer} (vs PC)`;
...
function handleCellClick(event) {
    ...
    // Gestione Mossa Online Multiplayer
    if (gameMode === 'online') {
        if (currentPlayer !== myRole) {
            showMessageBox("Non è il tuo turno!", "warning");
            return;
        }
        ...
        // Sincronizza la mossa su Firestore
        window.db.collection('games').doc(onlineGameId).update({
            board: board,
...
function checkGameStatus() {
    ...
    for (let i = 0; i < winningConditions.length; i++) {
        ...
        if (a === b && b === c) {
            roundWon = true;
            winnerToken = a; // Identifica il vincitore reale dal gettone sulla riga
            break;
        }
    }
...
function switchPlayer() {
    ...
    if (gameMode === 'online') {
        statusDisplay.textContent = myRole === currentPlayer ? "È il tuo turno!" : "In attesa dell'avversario...";
    } else {
        statusDisplay.textContent = `È il turno di ${currentPlayer}`;
    }
...
function endGame(winner) {
    ...
    // Se la partita online è conclusa, aggiorna lo stato anche su Firebase
...
resetGameBtn.addEventListener('click', () => {
    if (gameMode === 'online') {
        showMessageBox("Non puoi resettare una partita online in corso.", "warning");
        return;
    }
    initGame();
});
...
closeResultModalBtn.addEventListener('click', () => {
    ...
    if (gameMode === 'online') {
        gameMode = 'player-vs-player'; // Reset modalità dopo partita online
        initGame();
    }
});
